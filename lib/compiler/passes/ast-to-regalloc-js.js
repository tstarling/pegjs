...
function generateJavascript(ast, options) {
  ...
  var generate = visitor.build({
    ...
    literal: function(node, context) {
      ...
      if (node.value.length == 1 && !node.ignoreCase) {
        ...
        needLowerInput = true;
        result.condition = 'peg$lowerInput.charCodeAt(peg$currPos) === ' +
          node.value.charCodeAt(0);
        result.onSuccess([[reg, ' = input.charAt(peg$currPos);'].join('')]);
      } else {
        if (node.ignoreCase) {
          needLowerInput = true;
          result.block.push([reg, ' = ',
            'peg$lowerInput.substr(peg$currPos,', node.value.length, ');'].join(''));
          result.condition = [reg, ' === ', JSON.stringify(node.value)].join('');
  ...
  if (needLowerInput) {
    parts.push('  peg$lowerInput = input.toLowerCase();');
  }
