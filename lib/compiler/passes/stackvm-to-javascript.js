...
function generateJavascript(ast, options) {
  ...
  var cacheInitHook;
  if (options.cache) {
    var cacheInitHook = options.cacheInitHook || initCache;
    var cacheInitCode = indent4(cacheInitHook({
      ruleCount: ast.rules.length,
      variantCount: 2
    }));
  }
  ...
  parts.push(
    indent4(readSource('../../runtime/common-helpers')),
    indent4(readSource('../../runtime/stackvm-helpers')));
