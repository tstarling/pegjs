...
function generateJavascript(ast, options) {
  ...
  var functions = {
      ...
      if (node.alternatives.length == 1) {
        ...
        for (i = 0; i < node.alternatives.length; i++) {
          ...
          if (i != node.alternatives.length - 1) {
            block.push('  if (' + reg + ' !== peg$FAILED) break;');
          }
          free = free.concat(subresult.free);
        }
        block.push('} while (false);');
        return {
          block: block,
          expression: reg,
          free: free.concat(subresult.free)
    ...
    zero_or_more: function(node, context) {
      ...
      block.push(indent2(subresult.block.join('\n')));
      block.push(
        ['  if (', partReg, ' === peg$FAILED) break;'].join(''),
    ...
    one_or_more: function(node, context) {
      var free = [];
      var resultReg = context.getResultReg(free);
      var subresult = functions.zero_or_more(node, context);
      var block = subresult.block.concat([
        ['  if (', resultReg, '.length < 1) ', resultReg, ' = peg$FAILED;'].join('')
      ]);
      return {
        block: block,
        expression: resultReg,
  ...
  parts.push(
    ...
    '',
    '    if (peg$result[0] !== peg$FAILED && peg$result[1] === input.length) {',
    '      return peg$result[0];',
    '    } else {',
    '      if (peg$result[0] !== peg$FAILED && peg$result[1] < input.length) {',
    '        peg$failAtPosition(peg$result[1], { type: "end", description: "end of input" });',
    '      }',
    '',
