...
function generateJavascript(ast, options) {
  ...
  function makeActionFunc(code, env) {
    var code = '(' + objects.keys(env).join(', ') + ') {\n'
        + code + '\n' + '}';
    if (code in actionFuncIndexes) {
      return actionFuncIndexes[code];
    } else {
      var index = funcs.length;
      funcs.push('function peg$' + index + code);
      return 'peg$' + index;
    }
  }

  function makeActionFuncArgs(code, env) {
    return [makeActionFunc(code, env), JSON.stringify(objects.values(env))];
  }
  ...
  var generate = visitor.build({
    ...
    rule: function(node) {
      ...
      labelIndex = 1;
      return [
        'function peg$parse' + node.name + '(s) {',
    ...
    action: function(node, env) {
      ...
      return makeStdCall('peg$action',
        makeActionFuncArgs(node.code, newEnv).concat([jsExpr]),
        [], env);
    },
    ...
    zero_or_more: function(node, env) {
      ...
      if (node.expression.type == "literal") {
        ...
        return makeStdCall('peg$repeatLiteral',
          ['/^(?:' + js.regexpLiteralEscape(value) + ')*/', JSON.stringify(value)],
          [], env);
      }
      ...
      if (node.expression.type == "sequence"
          ...
          && node.expression.elements[0].type == "simple_not"
          && node.expression.elements[0].expression.type == "literal"
          && node.expression.elements[1].type == "any") {
        return makeStdCall('peg$consumeUpToLiteral',
            [JSON.stringify(node.expression.elements[0].expression.value)],
            [], env);
      }
    ...
    semantic_and: function(node, env) {
      return makeStdCall('peg$jsPredicate',
        makeActionFuncArgs(node.code, env).concat(['false']),
        [], env);
    },

    semantic_not: function(node, env) {
      return makeStdCall('peg$jsPredicate',
        makeActionFuncArgs(node.code, env).concat(['true']),
        [], env);
    },
